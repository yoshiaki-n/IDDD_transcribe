...
public class EventSourcedRootEntity {
    ...
    protected EventSourcedRootEntity(List<DomainEvent> anEventStream,
            int aStreamVersion) {
    ...
    protected void mutateWhen(DomainEvent aDomainEvent) {
        ...
        try {
            ...
            if (e.getCause() != null) {
                throw new RuntimeException("Method " + MUTATOR_METHOD_NAME
                        + "(" + eventType.getSimpleName()
                        + ") failed. See cause: " + e.getMessage(),
                        e.getCause());
            }

            throw new RuntimeException("Method " + MUTATOR_METHOD_NAME + "("
                    + eventType.getSimpleName() + ") failed. See cause: "
                    + e.getMessage(), e);
        ...
        synchronized (mutatorMethods) {
            try {
                ...
                throw new IllegalArgumentException(
                        "I do not understand " + MUTATOR_METHOD_NAME + "("
                                + anEventType.getSimpleName() + ") because: "
    ...
    private Method hiddenOrPublicMethod(
            Class<? extends EventSourcedRootEntity> aRootType,
            Class<? extends DomainEvent> anEventType) throws Exception {
